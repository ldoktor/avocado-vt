...
class HumanMonitor(Monitor):
    ...
    def set_migrate_capability(self, state, capability):
        ...
        :raise MonitorNotSupportedError if the capability is unknown
        """
        ...
        result =  self.cmd(cmd)
        if result != "":
            raise MonitorNotSupportedError("Failed to set capability %s: %s" %
                  (capability, result))
        return result
    ...
    def get_migrate_capability(self, capability):
        ...
        if (match is None):
            raise MonitorNotSupportedError("Unknown capability %s" % capability)
        value = match.group(1)
        return value == "on"
